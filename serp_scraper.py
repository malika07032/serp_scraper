...
def main(directory, file):
    serp_data = runFile(directory, file)
    path = file.split(".")[0] + ".json"
    output_file = path
    if not os.path.isdir("serp_contents"):
        os.mkdir('serp_contents')
    with open("serp_contents/"+str(output_file), "w") as outfile:
        json.dump(serp_data, outfile)
